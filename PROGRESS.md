...
| 2026-08-30 | **Tax-invoice + VAT module** (migration `0004_tax_invoices`, commits 770688a server / 4cafbaf web). `contacts` (SUPPLIER/CUSTOMER/BOTH, 13-digit tax id, branch), `invoices` + `invoice_items` (multi-line buy/sell), `doc_counters` (gapless `BUY-YYYY-NNNN` / `SELL-YYYY-NNNN` under a row lock — a rolled-back confirm reclaims the number), `movements.source_kind` += `INVOICE`, company profile in `settings`, products += `vat_applicable`/default prices. `services/invoices` create/update (DRAFT only, totals recomputed server-side), `confirmInvoice` (idempotent: assign number, one `postMovementTx` per line, freeze totals + contact snapshot, sum COGS for SELL), `voidInvoice` (reverse movements, recompute stock, keep number). `services/vatReports` = ภ.พ.30 รายงานภาษีซื้อ/ขาย per month. Per-line `vat_rate` 7 or 0 (Lao export zero-rated). Routes `/api/contacts`, `/api/invoices`(+confirm/void), `/api/vat-reports/:kind`; exports += `vat-purchase`/`vat-sales` .xlsx. Web: ContactsPage, InvoicesPage (list + multi-line editor), InvoicePrint (ใบกำกับภาษี, TH/EN, ต้นฉบับ/สำเนา, `window.print()`), VatReportPage (+ company-profile editor); 3 nav tabs. `invoices.test.ts` 8 (VAT math incl. zero-rated line, gapless numbering incl. no-burn-on-failed-confirm, idempotent confirm, void reversal, edit lock, report + CSV, raw-SQL cross-check). 81 server tests green (83 real PG); verified live end to end. |
...
| 2026-08-30 | **Customisable invoice print format** (migration `0005_print_settings`). One `print_settings` JSON blob in `settings`, shape + defaults enforced by `printSettingsSchema` in `@inventory/shared` and re-validated in `updateSettings` (bad blob → `VALIDATION_FAILED`). `services/printSettings.getPrintSettings` (defaults if row absent/invalid); `getInvoiceDetail` now returns `printSettings` alongside `company`. New `format/bahtText` — Thai baht-in-words (เอ็ด/สิบ/ยี่สิบ + ล้าน grouping), 5 unit tests. `InvoicePrint.tsx` consumes the settings and gained an inline **ตั้งค่ารูปแบบพิมพ์** panel with live preview + save: paper size A4/A5, margin mm, base font px, header logo (data URL, ~500 KB cap), section toggles (logo / EN labels / Original-Copy badge / reference line / VAT summary line / baht-in-words / signature block), free-text footer. Bug fixed in passing: `InvoicesPage` fetched `pageSize=1000`/`500` against a server cap of 200 (`zPagination`) and swallowed the 422, leaving the product/contact pickers empty — now walks pages at 200 and surfaces errors. Tests: shared 110, server 81 (+2 skipped); lint + typecheck + `vite build` clean; verified end to end against the Docker container. |
